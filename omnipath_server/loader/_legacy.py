...
class Loader:
    ...
    def _load_table(self, tbl: str):
        ...
        _log(
            f'File not found: `{path.name}[{"|".join(self._compr)}]`; ' f"skipping table `{tbl}`.",
        )
...
class TableLoader:
    ...
    def _read(self) -> Generator[tuple, None, None]:
        ...
        _log(
            f"Opening `{self.path}` by " f"`{opener.__module__}.{opener.__name__}" f"(... {_misc.dict_str(args)})`...",
        )
        ...
        with opener(self.path, **args) as fp:
            ...
            for row in reader:
                ...
                yield tuple(row[column.name] for column in self.columns if column.name in row)
